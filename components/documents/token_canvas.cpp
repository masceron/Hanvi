...
void TokenCanvas::set_scroll_value(int val) {
...
void TokenCanvas::on_active_token_changed(uint32_t /*token_id*/) {
    viewport()->update();
}

void TokenCanvas::on_hovered_token_changed(uint32_t /*token_id*/) {
    viewport()->update();
}
...
void TokenCanvas::rebuild_paragraph_data() {
    ...
    for (size_t p = 0; p < doc->paragraphs.size(); ++p) {
        const auto& para = doc->paragraphs[p];
        ParagraphLayout pl;
        bool in_quote = false;
...
void TokenCanvas::update_scroll_range() {
...
size_t TokenCanvas::find_paragraph_at_y(qreal doc_y) const {
    ...
    auto it = std::lower_bound(layouts_.begin(), layouts_.end(), doc_y,
...
void TokenCanvas::paintEvent(QPaintEvent* event) {
    ...
    const auto norm_sel = normalized_selection();

    painter.setFont(font_);

    for (size_t p = 0; p < layouts_.size(); ++p) {
        const auto& pl = layouts_[p];
        if (pl.y + pl.height < view_top || pl.y > view_bottom) continue;
        if (!pl.layout_valid || !pl.text_layout) continue;

        const qreal draw_y = pl.y - scroll_y;

        QList<QTextLayout::FormatRange> format_ranges;

        // 1. Character selection range (blue highlight)
        if (has_sel && p >= norm_sel.start.paragraph && p <= norm_sel.end.paragraph) {
            const int start_c = (p == norm_sel.start.paragraph) ? norm_sel.start.char_index : 0;
            const int end_c = (p == norm_sel.end.paragraph) ? norm_sel.end.char_index : static_cast<int>(pl.text.length());
        ...
        pl.text_layout->draw(&painter, QPointF(margin_, draw_y), format_ranges);
    }
}
...
void TokenCanvas::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        const auto hit = char_at_pos(event->pos(), false);
        if (hit) {
            is_selecting_ = true;
            sel_start_ = CharPosition{hit->first, hit->second};
            sel_end_ = sel_start_;
        } else {
            clear_selection();
...
void TokenCanvas::mouseMoveEvent(QMouseEvent* event) {
    if (is_selecting_) {
        ...
        if (has_dragged_) {
            ...
            const auto hit = char_at_pos(event->pos(), true);
            if (hit) {
                sel_end_ = CharPosition{hit->first, hit->second};
                viewport()->update();
            }
            return;
        }
    ...
    const auto hit = char_at_pos(event->pos(), false);
    if (hit) {
        const auto* span = token_at_char(hit->first, hit->second);
        if (span && session_) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (hit) {
            ...
            if (span && span->length > 0) {
                sel_start_ = CharPosition{hit->first, span->start_char};
                sel_end_ = CharPosition{hit->first, span->start_char + span->length};
                if (session_) {
                    session_->trigger_click(span->token_id);
                }
                viewport()->update();
...
void TokenCanvas::scroll_to_token(uint32_t token_id) {
    if (!session_ || !session_->has_document()) return;

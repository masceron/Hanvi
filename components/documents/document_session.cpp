...
void DocumentSession::trigger_context_menu(const uint32_t clicked_token_id)
{
    if (!doc_) return;
    ...
    if (has_selection())
    {
        const auto [start, end] = selection_.normalized();
        for (size_t p = start.paragraph; p <= end.paragraph && p < doc_->paragraphs.size(); ++p)
        {
            ...
            {
                if (const auto& tok = tokens[t]; tok.is_rule())
                {
        ...
        if (const QString cn_text = selected_chinese_text(); !cn_text.isEmpty())
        {
